...
- **Real-data validation pending:** genuine conclusions, metrics, ranges, and coverage require an approved real dataset.
- **GitHub publication prepared:** local repository and deployment artifacts are ready; remote creation/push requires an authenticated GitHub CLI session.
- **Public deployment pending:** Streamlit Community Cloud requires one-time owner browser authorization after publication.

Current data decision: no clearly licensed property-level candidate passed provenance and reuse review on 13 July 2026. The synthetic demonstration remains the publishable version; see `docs/REAL_DATASET_ASSESSMENT.md`.
