...
The repository contains a source-independent individual-property cleaning core and a generic real aggregate validation pipeline. The current completed-transaction inputs cover all 16 jurisdictions; source facts are supplied through metadata and the core record/pipeline is state-neutral. Aggregate rows must not be treated as individual records.
