...
def train_synthetic_fixture(*, count: int = 240, seed: int = 42,
               output_dir: str | Path | None = None,
               records: Iterable[dict[str, Any]] | None = None):
    """Train a deterministic test fixture from supplied synthetic rows or the generator."""
    ...
    if not source_rows or not all(bool(row.get("is_synthetic")) for row in source_rows):
        raise ValueError("synthetic fixture training accepts only clearly labelled synthetic records")
    ...
    report.update({
        "dataset_name": "Deterministic synthetic property fixture",
        "dataset_version": "synthetic-demo-v1",
        "synthetic": True,
        "model_version": "synthetic-demo-model-v1",
    ...
    paths={}
    if output_dir:
        out=Path(output_dir);out.mkdir(parents=True,exist_ok=True);bundle.save(out/"model_bundle.pkl");paths=write_evaluation(report,out/"evaluation")
