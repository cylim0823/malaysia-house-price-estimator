...
def _write_demo_records(path: str | Path, records: list[dict]) -> None:
...
def _read_demo_records(path: str | Path) -> list[dict]:
    source = Path(path)
    ...
    if not isinstance(records, list) or not all(isinstance(row, dict) for row in records):
        raise ValueError("demo training input must contain a list of records")
    for row in records:
...
def build_parser():
    ...
    t=sub.add_parser("train-demo");t.add_argument("--output-dir",required=True);t.add_argument("--input");t.add_argument("--count",type=int,default=240);t.add_argument("--seed",type=int,default=42)
...
def main(argv=None):
    ...
    try:
        if args.command=="generate-demo-data":_write_demo_records(args.output,generate_synthetic_records(args.count,seed=args.seed,price_type=args.price_type));print(SYNTHETIC_LABEL)
        ...
        elif args.command=="train-demo":bundle,summary,_=train_demo(count=args.count,seed=args.seed,output_dir=args.output_dir,records=_read_demo_records(args.input) if args.input else None);print(SYNTHETIC_LABEL);print(json.dumps(summary,indent=2))
