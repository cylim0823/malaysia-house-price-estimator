...
DATASET_VERSION = "penang-completed-transaction-aggregates-2017-v2"
SOURCE_NAME = "Penang State Government residential transaction tables"
SOURCE_URL = (
    "https://archive.data.gov.my/data/en_US/dataset/"
    "pecahan-bilangan-pindah-milik-harta-kediaman-mengikut-jenis-dan-daerah-di-pulau-pinang"
)
SOURCE_VALUE_URL = (
    "https://archive.data.gov.my/data/en_US/dataset/"
    "pecahan-bilangan-pindah-milik-harta-kediaman-mengikut-jenis-dan-daerah-rm-juta-di-pulau-pinang"
)
SOURCE_DOCUMENT = "Penang residential transaction counts and values by property type and district, 2017"
SOURCE_TABLE = "Quarter/property-type/district tables"
...
def validate_aggregate_frame(frame: pd.DataFrame) -> AggregateProcessingResult:
...
def load_and_validate_aggregate_csv(path: str | Path) -> AggregateProcessingResult:
    return validate_aggregate_frame(pd.read_csv(path))
...
class AggregateTransactionBundle:
    ...
    def predict(
        ...
        *,
        ...
        estimate = float(self.model.predict(frame)[0]) if quarter == 4 else None
        nearby = [
            value
        ...
        return {
            ...
            "available_date_range": "2017 Q1-Q4",
...
def train_aggregate_baselines(
    ...
    """Evaluate Q4 once after fitting weighted baselines on Q1-Q3."""
    eligible = processed[processed["model_eligible"]].copy()
    if eligible["year"].nunique() != 1:
        raise ValueError("this provisional baseline workflow expects the single-year dataset")
    train = eligible[eligible["quarter"] < 4].reset_index(drop=True)
    test = eligible[eligible["quarter"] == 4].reset_index(drop=True)
    ...
    report = {
        ...
        "temporal_limitation": (
            "Only 2017 is available. Q1-Q3 to Q4 evaluation is provisional and does not "
            "validate current or multi-year forecasting."
        ),
        "split_strategy": "train_2017_q1_q3_test_2017_q4",
